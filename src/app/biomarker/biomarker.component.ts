...
import { Biomarker, BiomarkerUnit, BiomarkerUnitType, isStringValue, validateEntry } from '../shared/biomarker';
...
export class BiomarkerComponent implements OnInit, AfterViewInit, DoCheck {
	...
	ngOnInit() {
		...
		const enums = this.biomarker.selectedUnit?.enum
			? new Map<string, string>(this.biomarker.selectedUnit?.enum?.map(obj => [obj, obj]))
			: undefined;
		const values = this.biomarker.selectedUnit?.values
			? new Map<string, string>(this.biomarker.selectedUnit?.values?.map(obj => [obj, obj]))
			: undefined;
