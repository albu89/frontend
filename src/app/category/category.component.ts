...
export class CategoryComponent implements OnChanges, OnDestroy {
  ...
  public ngOnChanges(changes: SimpleChanges): void {
    if (changes['biomarkers'] && this.biomarkers) {
      ...
      this.fixedBiomarkers = this.biomarkers.filter(b => FIXED_CATEGORIES.includes(b.category)).sort((a, b) => a.orderNumber - b.orderNumber);
      this.flexibleBiomarkers = this.biomarkers.filter(b => !this.fixedBiomarkers.includes(b)).sort((a, b) => a.orderNumber - b.orderNumber);
  ...
  protected savePreferences(): void { //todo
    if (!this.isEditingEnabled) {
      return;
    }

    // Format preferences for updating
    const preferences = this.flexibleBiomarkers.reduce((o, key, currentIndex) => ({ //todo type
      ...o, [key.id]: {orderNumber: this.smallestOrderNo + currentIndex, preferredUnit: key.selectedUnit.unitType}
    }), {})
